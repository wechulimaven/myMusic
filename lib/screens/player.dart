...
import 'package:theHit/widgets/scroll.dart';
import 'SongsListsScreen.dart';
...
class SlidingScaffoldState extends State<SlidingScaffold>
  ...
  void initState() {
    ...
    _albumArtController =
        PageController(initialPage: playingIndex, viewportFraction: 0.8);
  }

  void updatePreviousScreen(
      Song song, int index, List<Song> songs, bool isPlaying) {
    ScopedModel.of<SongModel>(context)
        .updateSong(song, index, songs, isPlaying);
  }
  ...
  Future prev() async {
    if (playingIndex != 0) {
      ...
      audioPlayer.stop();
      if (!paused) audioPlayer.play(song.uri);
                          ...
                          children: <Widget>[
                            ...
                            GestureDetector(
                              onHorizontalDragEnd: (drag) {
                                double v = drag.velocity.pixelsPerSecond.dx /
                                    size.width;
                                if (v < 0) {
                                  next();
                                } else {
                                ...
                                child: Container(
                                  ...
                                  child: ClipRRect(
                                    ...
                                    child: Image(
                                      ...
                                      fit: BoxFit.cover,
                                      image:
                                          (songs[playingIndex].albumArt != null)
                                              ? FileImage(File(
                                                  songs[playingIndex].albumArt))
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: () async {
                                              if (song.isFav == 0) {
                                                ...
                                                Provider.of<SongsListScreenState>(
                                                        context)
                                                    .updateFavList();
                                              } else {
                                                setState(() {
  ...
  void _animateToInitial() {
    _bottomSheetController.animateTo(0.05,
        duration: Duration(milliseconds: 250));
  }
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    double bottomSheetUpdatedValue =
        _bottomSheetController.value - dragSpeedToScreenSizeRatio;
  ...
  void _handleDragEnd(DragEndDetails details) {
    if (_bottomSheetController.isAnimating ||
        _bottomSheetController.status == AnimationStatus.completed) {
      return;
    }
  ...
  Widget _buildBottomSheetContent() {
    ...
    double topMarginAnimatedValue =
        (1 - _bottomSheetController.value) * topPaddingMax * 1;
                ...
                child: IntrinsicHeight(
                  child: Stack(
                    children: <Widget>[
                      ...
                      ((playingIndex + 1) >= songs.length &&
                              listContainerHeight <= 110)
                          ? SizedBox()
                          : Padding(
                              padding: const EdgeInsets.only(top: 40),
                              child: Container(
                                height: (listContainerHeight <= 50)
                                    ? 0
                                    : listContainerHeight,
                                width: MediaQuery.of(context).size.width,
                                child: ListView.builder(
                                  itemCount:
                                      this.songs.length - this.selectedIndex,
                                  itemBuilder:
                                    ...
                                    return ListTile(
                                      leading: CircleAvatar(
                                        ...
                                        child: ClipRRect(
                                          ...
                                          child:
                                              (songs[this.selectedIndex + index]
                                                          .albumArt !=
                                                      null)
                                                  ? Image(
                                                      height: 30,
                                      ...
                                      onTap: () {
                                        if (this.selectedIndex !=
                                          ...
                                          audioPlayer.stop();
                                          audioPlayer.play(song.uri);
                                          updatePreviousScreen(song,
                                              playingIndex, songs, !paused);
                                        }
                                      },
                                    );
                                  },
  ...
  slideContent({Widget content}) {
    ...
    final slideAmount =
        (MediaQuery.of(context).size.width * 0.8) * slidePercent;
  ...
  Widget build(BuildContext context) {
    ...
    nextHasAlbumArt = ((playingIndex + 1) < songs.length - 1 &&
        songs[playingIndex + 1].albumArt != null);
...
class SlidingScaffoldMenuControllerState
    extends State<SlidingScaffoldMenuController> {
  @override
  Widget build(BuildContext context) {
...
typedef Widget SlidingScaffoldBuilder(
